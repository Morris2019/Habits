...
class HomeCollectionViewController: UICollectionViewController {
    ...
    struct Model {
        ...
        var userStatistics = [UserStatics]()
    ...
    func updateCollectionView() {
        ...
        let currentUserLoggedHabits = loggedHaabitNames(for: model.currentUser)
        let favoriteLoggedHabites = Set(model.favoriteHabits.map { $0.name }).intersection(currentUserLoggedHabits)
        
        // Loop through all the followed users.
        for followedUser in model.followedUsers.sorted(by: { $0.name < $1.name }) {
            let message: String
            
            let followedUserLoggedHabits = loggedHaabitNames(for: followedUser)
            ...
            if commonLoggedHabits.count > 0 {
                ...
                let commonFavoriteLoggedHabits = favoriteLoggedHabites.intersection(commonLoggedHabits)
    ...
    func loggedHaabitNames(for user: User) -> Set<String> {
        var names = [String]()
